...
if __name__ == "__main__":
    ...
    # Visualize some data
    visualize_model(samples_train, 1, train_data, model, train_data.num_keypoint)
    # Optimizer
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % 10 == 0:
            torch.save(
                model.state_dict(),
                os.path.join(
                    "models/", args.exp_name, f"field_keypoint_best_{epoch}.pd"
                ),
            )
    ...
    # Visualize results
    visualize_model(samples_val, 5, val_data, model, train_data.num_keypoint)
    visualize_model(samples_train, 5, train_data, model, train_data.num_keypoint)
