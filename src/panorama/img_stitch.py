...
def fitting_rectangle(points):
    points_cp = np.array(points.copy())
    l, t = np.min(points_cp, axis=0)
    r, b = np.max(points_cp, axis=0)
    l = int(math.floor(l))
    t = int(math.floor(t))
    # print((l, t), (int(math.ceil(b - t)), int(math.ceil(r - l))))
    return (l, t), (int(math.ceil(b - t)), int(math.ceil(r - l)))
...
class _StitchImage:
    ...
    def find_features(self):
        self.kp, self.feat = self.sift.detectAndCompute(self.image, None)
...
class ImageStitcher:
    def __init__(self, initial_frames):
        ...
        for i, frame in enumerate(initial_frames):
            self.add_image(frame, 'frame%d' % i)
    ...
    def center(self) -> int:
        if self.center_ is None:
            self.center_ = self._find_center()
        return self.center_
    ...
    def stitch(self):
        ...
        canvas = np.zeros((base_size[0] * 2, base_size[1] * 2, 4), dtype=np.uint8)
        h, w = self._images[0].image.shape[0], self._images[0].image.shape[1]
        for j in order:
            ...
            if size[0] > h * 2.75 or size[1] > w * 2.75 or size[0] < h / 2.75 or size[1] < w / 2.75:
                print('failed', size[0], size[1])
                continue
            T = np.array([[1, 0, -shift[0]], [0, 1, -shift[1]], [0, 0, 1]])
            Ht = T.dot(H)
            # print('Translated homography:\n', Ht)
            new_image = cv2.warpPerspective(image.image, Ht, tuple(size))
    ...
    def calculate_draw_order(self, parents):
        order = csgraph.depth_first_order(csgraph.reconstruct_path(self._edge_matrix, parents, directed=False),
                                          self.center, return_predecessors=False)[::-1]
        # print('Order to Draw:')
        strf = ''
        for i in order:
    ...
    def calculate_relative_homographies(self, parents):
        ...
        for src_idx, dst_idx in enumerate(parents):
            if dst_idx < 0 or src_idx == c:
                ...
                continue
            matches = self._matches[(src_idx, dst_idx)] if (src_idx, dst_idx) in self._matches else self._matches[
                (dst_idx, src_idx)]
    ...
    def _find_homography(self, src: _StitchImage, dst: _StitchImage, matches: List[cv2.DMatch],
    ...
    def _edge_matrix(self):
        ...
        self.current_edge_matrix = csr.csr_matrix((values, tuple(np.array(all_matches).T)),
...
from stitching_utils.extraction import Video, read_images

if __name__ == '__main__':
    key_frames_path = './key_frames/*.png'
    images = read_images(key_frames_path)
    for i in range(len(images)):
        images[i] = cv2.cvtColor(images[i], cv2.COLOR_BGR2RGB)
    stitch = ImageStitcher(images)
    pano = stitch.stitch()
    cv2.imshow('result', cv2.cvtColor(pano, cv2.COLOR_BGR2RGB))
    cv2.waitKey()
